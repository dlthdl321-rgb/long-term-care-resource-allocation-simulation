...
export function actionGuidance(
  ...
  if (selected.resource_type === "핵심인력" && n(selected.relative_shortage_score) > 0) {
    items.push({
      signal: "서비스 제공인력 부족",
  ...
  if (selected.resource_type === "정원" && n(selected.relative_shortage_score) > 0) {
    items.push({
      signal: "주야간보호 정원 부족",
...
export function comparePreset(
  ...
  const current = n(selected.current_resource);
  const demand = n(selected.demand_value);
  const target = n(selected.target_resource);
  const after = Math.max(0, current + delta);
  const gapBefore = Math.max(0, target - current);
  const gapAfter = Math.max(0, target - after);
  const supplyAfter = demand > 0 ? (after / demand) * 1000 : 0;
  const shortageAfter =
    n(selected.target_supply_level) > 0
      ? 1 - Math.min(1, supplyAfter / n(selected.target_supply_level))
      : 0;
  return {
    after,
    gapAfter,
