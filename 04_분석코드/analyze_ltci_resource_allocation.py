"""장기요양 데이터 감사와 기준선 공급지표 생성 파이프라인.

이 스크립트는 다음 순서로 실행한다.
1. 입력 파일·컬럼·키·결측·중복 감사
2. 변수 사전과 기술통계 생성
3. 시군구×서비스 유형 핵심 지표 계산
4. 이전 실행 재현용 탐색 가설·단순 배치 단계(레거시 호환)

주의:
- 기본 실행은 데이터 감사만 수행한다.
- 최종 가설검정은 analyze_hypothesis_testing.py, 대표 배치 결과는
  simulation/run_allocation_scenarios.py가 각각 단일 기준 구현이다.
- hypotheses·simulation·all 단계는 과거 산출물 재현을 위한 호환 경로이며
  현재 대표 결과 생성에 사용하지 않는다.
